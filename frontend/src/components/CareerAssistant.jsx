...
export default function CareerAssistant({ currentProfile = null, currentResult = null }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6 animate-fadeIn pb-12">
      {/* Header Panel */}
      <div className="glass-panel p-6 sm:p-8 rounded-xl border border-[#D8C8B5] bg-[#EDE3D4] relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-4 shadow-luxury">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="px-2 py-0.5 rounded bg-[#FBF8F2] border border-[#D8C8B5] text-[#9F595B] text-[10px] uppercase font-bold tracking-wider">
                AI Advice (Separated from ML Model)
              </span>
            </div>
        ...
        <button
          type="button"
          onClick={clearChat}
          className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-[#FBF8F2] hover:bg-[#F5EFE6] text-[#30241D] border border-[#D8C8B5] transition flex items-center space-x-1.5 cursor-pointer shadow-card"
        >
      ...
      <div className="glass-panel rounded-xl border border-[#D8C8B5] bg-[#EDE3D4] flex flex-col h-[560px] overflow-hidden relative shadow-luxury">
        {/* Chat Messages Stream */}
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4 no-scrollbar bg-[#F5EFE6]/60">
          {messages.map((msg, idx) => (
            ...
            >
              ...
              <div className={`max-w-[82%] sm:max-w-[75%] space-y-1 ${
                ...
                <div className={`p-4 rounded-xl text-xs sm:text-sm leading-relaxed whitespace-pre-wrap shadow-card ${
                  msg.sender === "user"
                    ? "bg-[#9F595B] text-white"
                    : msg.isError
                ...
                <div className="flex items-center space-x-2 text-[10px] text-[#756354] px-1">
                  <span>{msg.time}</span>
                  {msg.sender === "bot" && (
                    <span className="text-[10px] text-[#9F595B] font-mono">
          ...
          {loading && (
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 rounded-lg bg-[#FBF8F2] border border-[#D8C8B5] text-[#9F595B] flex items-center justify-center">
                <FaRobot className="text-xs" />
              </div>
              <div className="p-3 rounded-xl bg-[#FBF8F2] border border-[#D8C8B5] text-xs text-[#756354] flex items-center space-x-2">
                <FaSpinner className="animate-spin text-[#9F595B]" />
                <span>Analyzing student profile & formulating career advice...</span>
              </div>
            </div>
          )}
        ...
        <div className="p-3 bg-[#EDE3D4] border-t border-[#D8C8B5] flex items-center gap-2 overflow-x-auto no-scrollbar">
          <span className="text-[10px] uppercase font-bold text-[#756354] whitespace-nowrap pl-2">
            Suggested:
          </span>
          {SUGGESTED_PROMPTS.map((prompt, idx) => (
            <button
              ...
              disabled={loading}
              className="text-xs px-3 py-1.5 rounded-lg bg-[#FBF8F2] hover:bg-[#F5EFE6] text-[#30241D] border border-[#D8C8B5] transition whitespace-nowrap cursor-pointer disabled:opacity-50 font-semibold shadow-card"
            >
              {prompt.label}
            </button>
          ))}
        ...
        {/* Chat Input Bar */}
        <div className="p-4 bg-[#EDE3D4] border-t border-[#D8C8B5] flex items-center space-x-3">
          <input
            type="text"
            value={inputMessage}
            onChange={(e) => setInputMessage(e.target.value)}
            onKeyDown={handleKeyPress}
            placeholder="Ask a question (e.g., 'What should I study today?', 'Give me a 7-day plan')..."
            className="flex-1 bg-[#F5EFE6] border border-[#D8C8B5] rounded-xl px-4 py-3 text-xs sm:text-sm text-[#30241D] placeholder-[#756354] focus:outline-none focus:border-[#9F595B] transition"
          />
          ...
          <button
            ...
            className="px-4 py-3 rounded-xl bg-[#9F595B] hover:bg-[#824647] text-white font-bold disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center justify-center cursor-pointer shadow-card"
          >
      ...
      {/* Disclaimer Banner */}
      <div className="p-3.5 rounded-lg bg-[#FBF8F2] border border-[#D8C8B5] text-xs text-[#756354] flex items-start space-x-2 shadow-card">
        <FaInfoCircle className="text-[#9F595B] text-sm flex-shrink-0 mt-0.5" />
        <span>
