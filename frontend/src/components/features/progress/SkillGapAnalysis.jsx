  ...
  return (
    <div className="w-full mt-6 sm:mt-8 space-y-4 sm:space-y-6">
      ...
      <div className="glass-panel p-4 sm:p-6 md:p-8 rounded-xl border border-[#D8C8B5] bg-[#EDE3D4] relative overflow-hidden shadow-luxury">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[11px] sm:text-xs text-[#756354] mt-1 max-w-2xl">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
            <div className="px-2.5 sm:px-3 py-1 rounded bg-[#FBF8F2] border border-[#D8C8B5] text-[#71856B] text-[11px] sm:text-xs font-semibold flex items-center space-x-1.5 shadow-card">
              <span className="h-2 w-2 rounded-full bg-[#71856B]"></span>
              <span>{strongCount} Strong</span>
            </div>
            <div className="px-2.5 sm:px-3 py-1 rounded bg-[#FBF8F2] border border-[#D8C8B5] text-[#B9935A] text-[11px] sm:text-xs font-semibold flex items-center space-x-1.5 shadow-card">
              <span className="h-2 w-2 rounded-full bg-[#B9935A]"></span>
              <span>{moderateCount} Moderate</span>
            </div>
            <div className="px-2.5 sm:px-3 py-1 rounded bg-[#FBF8F2] border border-[#D8C8B5] text-[#A65D5D] text-[11px] sm:text-xs font-semibold flex items-center space-x-1.5 shadow-card">
        ...
        {/* Disclaimer Notice */}
        <div className="mt-3.5 sm:mt-4 pt-3 border-t border-[#D8C8B5] flex items-start space-x-2 text-[11px] sm:text-xs text-[#756354] leading-relaxed">
      ...
      {topPriorities && topPriorities.length > 0 && (
        <div className="glass-card p-4 sm:p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] shadow-card">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center space-x-2.5 sm:space-x-3">
              <div className="p-2 sm:p-2.5 rounded-lg bg-[#F5EFE6] border border-[#D8C8B5] text-[#B9935A] flex-shrink-0">
            ...
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
              {topPriorities.map((item, idx) => (
                <div
                  key={idx}
                  className="px-2.5 sm:px-3 py-1 rounded bg-[#F5EFE6] border border-[#D8C8B5] text-[11px] sm:text-xs font-bold text-[#B9935A] flex items-center space-x-1.5"
                >
                  <span className="h-4 w-4 rounded-full bg-[#EDE3D4] text-[#30241D] flex items-center justify-center text-[9px] sm:text-[10px]">
                    {idx + 1}
                  </span>
                  <span>{item}</span>
                </div>
      ...
      {/* Filter Tabs */}
      <div className="flex items-center justify-between flex-wrap gap-2 pt-1 sm:pt-2">
        <div className="flex items-center space-x-1.5 sm:space-x-2 text-xs text-[#756354] font-semibold">
          <FaFilter className="text-[#9F595B] text-xs" />
          <span>Filter by Status:</span>
        </div>
        <div className="flex items-center gap-1 p-1 rounded-lg bg-[#EDE3D4] border border-[#D8C8B5] overflow-x-auto max-w-full">
          <button
            onClick={() => setFilter("ALL")}
            className={`px-2.5 sm:px-3 py-1 rounded text-[11px] sm:text-xs font-semibold transition cursor-pointer whitespace-nowrap ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
        {filteredSkills.map((item, idx) => {
          ...
          return (
            <div
              key={idx}
              className="glass-card p-4 sm:p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] space-y-2.5 sm:space-y-3 shadow-card"
            >
              {/* Header */}
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-2 sm:space-x-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h5 className="text-xs sm:text-sm font-bold text-[#30241D] tracking-tight truncate">
                      {item.skill}
                    </h5>
                    <span className="text-[9px] sm:text-[10px] text-[#756354] font-mono block truncate">
                ...
                <div className="text-right flex-shrink-0 ml-2">
                  <span className="text-xs sm:text-sm font-extrabold text-[#30241D] font-mono block">
                    {item.score_display}
                  </span>
                  <span className={`text-[9px] sm:text-[10px] uppercase font-extrabold px-1.5 sm:px-2 py-0.5 rounded inline-block mt-0.5 ${
                    isNeedsImp
              ...
              {/* Progress Bar */}
              <div className="space-y-1 my-1.5 sm:my-2">
              ...
              <div className="p-2.5 sm:p-3 rounded-lg bg-[#F5EFE6] border border-[#D8C8B5] flex items-start space-x-2">
                <FaLightbulb className={`text-[11px] sm:text-xs mt-0.5 flex-shrink-0 ${
                  isNeedsImp ? "text-[#A65D5D]" : isModerate ? "text-[#B9935A]" : "text-[#71856B]"
                }`} />
                <p className="text-[11px] sm:text-xs text-[#30241D] leading-snug">
                  {item.recommendation}
                </p>
              </div>
            </div>
