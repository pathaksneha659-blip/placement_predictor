...
import SkillGapAnalysis from "../progress/SkillGapAnalysis";
import DailyGoals from "../progress/DailyGoals";
    ...
    <div className="w-full max-w-5xl mx-auto space-y-6 sm:space-y-8 animate-fadeIn pb-12">
      ...
      <div className="glass-panel p-4 sm:p-6 md:p-8 rounded-xl border border-[#D8C8B5] bg-[#EDE3D4] relative overflow-hidden shadow-luxury">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <div className="inline-flex items-center space-x-1.5 sm:space-x-2 px-2.5 sm:px-3 py-1 rounded-md bg-[#FBF8F2] border border-[#D8C8B5] text-[10px] sm:text-xs font-bold uppercase tracking-wider text-[#9F595B] mb-2 sm:mb-3">
            <FaShieldAlt className="text-[#9F595B]" />
            <span>ML Inference Verified</span>
          </div>
          <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-[#30241D] tracking-tight font-serif">
            Placement Prediction Assessment
          </h2>
          <p className="text-[#756354] text-xs sm:text-sm mt-1 max-w-lg mx-auto px-2">
            Evaluated through our trained Logistic Regression & Feature Selection Pipeline.
          </p>
        </div>

        {/* Main Highlights Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-center mb-6 sm:mb-8">
          {/* Placement Probability Card */}
          <div className="glass-card p-4 sm:p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] text-center relative order-2 md:order-1 shadow-card">
            <div className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-[#71856B] mb-1">
              Placement Probability
            </div>
            <div className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#71856B] tracking-tight my-1 sm:my-2">
              {placedProb.toFixed(2)}%
            </div>
            <div className="w-full bg-[#F5EFE6] h-2 rounded-full overflow-hidden mt-2.5 sm:mt-3 border border-[#D8C8B5]">
              <div 
                className="bg-[#71856B] h-full rounded-full transition-all duration-700 ease-out"
                style={{ width: `${placedProb}%` }}
              />
            </div>
            <span className="text-[11px] sm:text-xs text-[#756354] mt-2 block">
              Likelihood of campus offer
            </span>
          </div>

          {/* Central Circular Gauge & Status */}
          <div className="glass-card p-4 sm:p-6 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] text-center flex flex-col items-center justify-center relative order-1 md:order-2 shadow-card">
            <div className="relative w-32 h-32 sm:w-40 sm:h-40 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-[9px] sm:text-[10px] uppercase font-bold text-[#756354] tracking-wider">
                  Status
                </span>
                <span className={`text-lg sm:text-xl font-extrabold tracking-tight mt-0.5 ${
                  isPlaced ? "text-[#71856B]" : "text-[#A65D5D]"
                }`}>
                  {result.placement_status.toUpperCase()}
                </span>
                {isPlaced ? (
                  <FaCheckCircle className="text-[#71856B] text-sm sm:text-base mt-1" />
                ) : (
                  <FaTimesCircle className="text-[#A65D5D] text-sm sm:text-base mt-1" />
                )}
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-4 sm:p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] text-center relative order-3 shadow-card">
            <div className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-[#A65D5D] mb-1">
              Not Placed Probability
            </div>
            <div className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#A65D5D] tracking-tight my-1 sm:my-2">
        ...
        <div className="p-3.5 sm:p-4 rounded-xl bg-[#FBF8F2] border border-[#D8C8B5] mb-5 sm:mb-6">
          <div className="flex items-center space-x-2 text-[#9F595B] font-bold text-xs sm:text-sm mb-2 sm:mb-2.5">
            <FaLightbulb className="text-[#B9935A] text-xs sm:text-sm" />
        ...
        <div className="flex justify-center">
          <button
            onClick={onReset}
            className="w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-xs sm:text-sm bg-[#9F595B] hover:bg-[#824647] text-white transition flex items-center justify-center space-x-2 cursor-pointer shadow-card"
          >
