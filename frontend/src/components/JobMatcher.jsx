...
export default function JobMatcher({ currentProfile }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8 animate-fadeIn pb-12">
      ...
      {/* Input Section */}
      <div className="glass-panel p-6 sm:p-8 rounded-xl border border-[#D8C8B5] bg-[#EDE3D4] space-y-6 shadow-luxury">
        {/* Preset Buttons */}
        <div className="space-y-2">
          <label className="text-xs font-semibold text-[#756354] uppercase tracking-wider block">
          ...
          <div className="flex flex-wrap gap-2">
            {PRESET_JOBS.map((preset, idx) => (
              <button
                ...
                className="text-xs px-3.5 py-1.5 rounded-lg bg-[#FBF8F2] hover:bg-[#F5EFE6] text-[#30241D] border border-[#D8C8B5] transition cursor-pointer font-semibold shadow-card"
              >
                {preset.title}
              </button>
            ))}
        ...
        {/* Textarea */}
        <div className="space-y-2">
          <label htmlFor="jd-input" className="text-sm font-semibold text-[#30241D] block">
          ...
          <textarea
            ...
            className="w-full bg-[#F5EFE6] border border-[#D8C8B5] rounded-xl p-4 text-xs sm:text-sm text-[#30241D] placeholder-[#756354] focus:outline-none focus:border-[#9F595B] transition"
          />
          <div className="flex justify-between text-[11px] text-[#756354]">
        ...
        {error && (
          <div className="p-3.5 rounded-lg bg-[#A65D5D]/15 border border-[#A65D5D] text-[#A65D5D] text-xs flex items-center space-x-2">
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="glass-card p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] space-y-3 shadow-card">
              <div className="flex items-center space-x-2 text-[#71856B] font-bold text-sm pb-2 border-b border-[#D8C8B5]">
                <FaCheckCircle className="text-base" />
                <span className="text-[#30241D]">Matched Skills ({result.matched_skills ? result.matched_skills.length : 0})</span>
              </div>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {result.matched_skills && result.matched_skills.length > 0 ? (
                  result.matched_skills.map((skill, idx) => (
                    <span key={idx} className="px-2.5 py-1 rounded bg-[#F5EFE6] border border-[#D8C8B5] text-[#71856B] text-xs font-semibold">
                      ✓ {skill}
                    </span>
                  ))
                ) : (
            ...
            <div className="glass-card p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] space-y-3 shadow-card">
              <div className="flex items-center space-x-2 text-[#A65D5D] font-bold text-sm pb-2 border-b border-[#D8C8B5]">
                ...
                <span className="text-[#30241D]">Missing Skills ({result.missing_skills ? result.missing_skills.length : 0})</span>
              </div>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {result.missing_skills && result.missing_skills.length > 0 ? (
                  result.missing_skills.map((skill, idx) => (
                    <span key={idx} className="px-2.5 py-1 rounded bg-[#F5EFE6] border border-[#D8C8B5] text-[#A65D5D] text-xs font-semibold">
                      ✕ {skill}
                    </span>
                  ))
                ) : (
            ...
            <div className="space-y-2 pt-1">
              {result.recommended_actions && result.recommended_actions.length > 0 ? (
                result.recommended_actions.map((act, idx) => (
                  <div key={idx} className="flex items-start space-x-2.5 text-xs text-[#30241D]">
                    <span className="h-5 w-5 rounded bg-[#F5EFE6] border border-[#D8C8B5] text-[#9F595B] font-bold flex items-center justify-center text-[11px] flex-shrink-0">
                      {idx + 1}
                    </span>
          ...
          <div className="p-3.5 rounded-lg bg-[#FBF8F2] border border-[#D8C8B5] text-xs text-[#756354] flex items-start space-x-2">
