  ...
  const normalizedItems = useMemo(
    () =>
      items.map((item, index) =>
        typeof item === 'string'
          ? { src: item, alt: `Spiral item ${index + 1}` }
          : { alt: `Spiral item ${index + 1}`, ...item }
      ),
    [items]
  );
    ...
    >
      <div className="infinite-spiral__stage" role="list" aria-label="Infinite spiral gallery">
        {normalizedItems.map((item, index) => {
          ...
          return (
            <Card
              key={item.id ?? `${item.src || item.title || index}-${index}`}
              ref={node => {
              ...
              role="listitem"
              aria-label={item.label ?? item.alt ?? item.title}
            >
              {item.src ? (
                <img
                ...
                <div className="flex flex-col items-center justify-center p-2 text-center w-full h-full select-none">
                  {item.icon && <div className="text-xl mb-1 text-violet-400">{item.icon}</div>}
                  {item.title && <span className="text-xs font-bold text-white tracking-tight leading-tight">{item.title}</span>}
                  {item.subtitle && <span className="text-[10px] text-cyan-300 font-mono mt-0.5">{item.subtitle}</span>}
                </div>
              )}
            </Card>
          );
