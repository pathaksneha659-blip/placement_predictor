...
export default function WhatIfSimulator({ currentProfile, currentResult }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto glass-panel p-6 sm:p-8 rounded-xl border border-[#D8C8B5] bg-[#EDE3D4] shadow-luxury">
      {/* Panel Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-6 border-b border-[#D8C8B5] gap-4">
        <div className="flex items-center space-x-3">
          <div className="p-2.5 rounded-lg bg-[#FBF8F2] border border-[#D8C8B5] text-[#9F595B]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-8 items-stretch">
        ...
        <div className="glass-card p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] flex flex-col justify-between text-center shadow-card">
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-[#756354] mb-1">
              CURRENT PROFILE
            </div>
            <div className="text-3xl font-extrabold text-[#30241D] my-2 font-mono">
          ...
          <p className="text-[11px] text-[#756354] mt-4 pt-3 border-t border-[#D8C8B5]">
            Baseline ML prediction
          </p>
        </div>

        {/* Delta Card */}
        <div className="glass-card p-5 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] flex flex-col items-center justify-center text-center shadow-card">
          <span className="text-[11px] font-bold uppercase tracking-wider text-[#756354] mb-1">
            SIMULATED DIFFERENCE
          </span>
          <div className={`text-3xl font-extrabold my-2 flex items-center space-x-1 font-mono ${
            isPositiveDelta ? "text-[#71856B]" : "text-[#A65D5D]"
          }`}>
            {isPositiveDelta ? <FaArrowUp className="text-lg mr-1" /> : <FaArrowDown className="text-lg mr-1" />}
            <span>{isPositiveDelta ? `+${delta.toFixed(2)}` : delta.toFixed(2)}</span>
          </div>
          <span className="text-xs font-medium text-[#756354]">
            percentage points
          </span>
        </div>

        {/* What-If Simulated Profile Card */}
        <div className="glass-card p-5 rounded-xl border border-[#9F595B]/40 bg-[#FBF8F2] flex flex-col justify-between text-center relative shadow-card">
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-[#9F595B] mb-1 flex items-center justify-center space-x-1">
      ...
      <div className="p-3.5 rounded-lg bg-[#FBF8F2] border border-[#D8C8B5] text-xs text-[#756354] mb-8 flex items-start space-x-2.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {SIMULATOR_FIELDS.map((field) => {
          ...
          return (
            <div key={field.key} className="glass-card p-4 rounded-xl border border-[#D8C8B5] bg-[#FBF8F2] space-y-3 shadow-card">
