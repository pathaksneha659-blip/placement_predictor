            ...
            >
              {item.src ? (
                ...
                <div className="flex flex-col items-center justify-center p-2 text-center w-full h-full select-none">
                  {item.icon && (
                    <div className="text-xl mb-1 text-[#9F595B]">
                      {typeof item.icon === 'function' ? <item.icon /> : item.icon}
                    </div>
                  )}
                  {item.title && <span className="text-xs font-bold text-[#30241D] tracking-tight leading-tight">{item.title}</span>}
                  {item.subtitle && <span className="text-[10px] text-[#756354] font-mono mt-0.5">{item.subtitle}</span>}
                </div>
              )}
            </Card>
          );
